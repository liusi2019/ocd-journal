...
def find_nearest_valid_distribution(u_alpha, kernel, initial=None, reg=0):
    ...
    #print("distance_sqd = ", distance_sqd)
    #print("solution['primal objective'] = ", solution['primal objective'])
    #print("u_alpha = ", u_alpha)
    #print("np.dot(u_alpha.T, np.dot(kernel, u_alpha))[0, 0] = ", np.dot(u_alpha.T,
    #        np.dot(kernel, u_alpha))[0, 0])
...
def mpe(kernel,N,M,nu,epsilon=0.04,lambda_upper_bound= 20.):
...
if __name__=='__main__':
    ...
    name of clean file,
    name of mixture file,
    flag, whether scores or not
    name of output file
    '''
    clean_name = sys.argv[1]
    mixture_name = sys.argv[2]
    flag = int(sys.argv[3])
    output_name = sys.argv[4]


    if flag:
        X_component = np.genfromtxt(clean_name,delimiter=',', skip_header = 1)
        X_mixture = np.genfromtxt(mixture_name,delimiter=',', skip_header = 1)
        ## need to change the length 20000
        length = len(X_component)

        X_component = X_component.reshape((length, 1))
        X_mixture = X_mixture.reshape((length, 1))
    else:
        X_component = np.genfromtxt(clean_name,delimiter=',', skip_header = 1)
        X_mixture = np.genfromtxt(mixture_name,delimiter=',', skip_header = 1)
    ...
    result = pd.DataFrame({'alpha_1': [1-KM1], 'alpha_2': [1-KM2]})
