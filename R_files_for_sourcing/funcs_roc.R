...
## this version commented out the setting random seed part in cpe
## July 26, add line 83 - 85, to deal with the case when the clean and mixture are too hard to tell from each other and 
## the scores are clustered in a narrow interval. In this case alpha, beta from auc function will all be 0, 1
...
kappahat <- function(posteriors, Y, n1, n2){
  ...
  if(sum(ind) == 0){ ## this happens when the clean and mixture are too hard to tell from each other
    return(1)
  }
  fp = fp[ind]
  tp = tp[ind]
